...
export default class FriendPage extends cc.Component {
        ...
        this.buttonList.children.forEach(c => {
            c.on(cc.Node.EventType.TOUCH_END, () => {
                if (c == this.buttonList.children[1] && !this.privateChatTarget) {
                    Camper.getInstance().showToast("Vui lòng chọn người bạn để chat");
                    return;
                }
                this.buttonList.children.forEach(c => {
                ...
                if (c == this.buttonList.children[0]) {
                    JCLib.getComponentByPath(this.node, "Window/Title/Label", cc.Label).string = "Danh Sách Bạn Bè";
                    ...
                    JCLib.getComponentByPath(this.node, "Window/Title/Label", cc.Label).string = "Chat Với Bạn Bè（" + this.privateChatTarget.nickname + "）";
                    this.showChildWindow(2);
                    this.renderPrivateMsg();
                } else if (c == this.buttonList.children[2]) {
                    JCLib.getComponentByPath(this.node, "Window/Title/Label", cc.Label).string = "Kênh Chat Thế Giới";
    ...
    renderFriends(friends: any[]) {
        ...
        for (let friend of friends) {
            ...
            item.getChildByName("Segment").getComponent(cc.Label).useSystemFont = true;
            item.getChildByName("Segment").getComponent(cc.Label).string = ResourceMgr.getSegmentByIntegral(friend.integral);
            item.getChildByName("Nickname").getComponent(cc.Label).useSystemFont = true;
            ...
            if (friend.newMsgCount > 0) {
                ...
                if (friend.newMsgCount < 100) {
                    msgNew.string = "Tin mới(" + friend.newMsgCount + ")";
                } else {
                    msgNew.string = "Tin mới(99+)";
                }
            }
            item.getChildByName("IconDelete").on(cc.Node.EventType.TOUCH_END, () => {
                GlobalData.player.call("FriendController.deleteFriend", [friend.id], (res) => {
                    if (res) {
                        item.destroy();
                        Camper.getInstance().showToast("Xóa thành công");
                    }
                });
            });
    ...
    renderPrivateMsg() {
        ...
        GlobalData.player.call("FriendController.loadFriendMsg", [friend_id], (res: FriendChat[]) => {
            ...
            for (let friendChat of list) {
                ...
                JCLib.getComponentByPath(item, "Nickname", cc.Label).useSystemFont = true;
                JCLib.getComponentByPath(item, "Nickname", cc.Label).string = nickname;
                JCLib.getComponentByPath(item, "Label", cc.Label).useSystemFont = true;
    ...
    sendPrivateMsg() {
        ...
        if (!input.string.trim()) {
            Camper.getInstance().showToast("Nội dung chat không được để trống");
            return;
        }
    ...
    renderAddFriendPanel(friendChatPublic: FriendChatPublic) {
        ...
        JCLib.getComponentByPath(panel, "Layout/Layout/Layout1/Segment", cc.Label).useSystemFont = true;
        ...
        JCLib.getComponentByPath(panel, "Layout/Layout/Layout1/Nickname", cc.Label).useSystemFont = true;

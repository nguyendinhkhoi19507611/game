...
export default class EquipmentSelect extends cc.Component {
    ...
    clearBag() {
        ...
        this.bagContentItemCount.useSystemFont = true;
        this.bagContentItemCount.string = "Số lượng:";
    }
    ...
    clearPanel() {
        ...
        this.detail_starLevel.useSystemFont = true;
        this.detail_starLevel.string = "Cấp Sao:";
        this.detail_strengthLevel.useSystemFont = true;
        this.detail_strengthLevel.string = "Cường Hóa:";
    ...
    clickTopBarBtn(index: number) {
        let vietnameseNames = ["Hộ Mệnh", "Vòng Tay", "Mũ", "Dây Chuyền", "Nanh", "Đá"];
        this.topBarBtns.children.forEach((c, i) => {
            c.getChildByName("Label").getComponent(cc.Label).string = vietnameseNames[i];
            c.getChildByName("Label").getComponent(cc.Label).useSystemFont = true;
            if (i != index) {
                c.active = false;
            }
        });
        ...
        for (let userEquipmentInfo of GlobalData.userEquipmentInfos) {
            // Filter by ID prefix instead of name suffix
            // IDs are like 3001 (Slot 0), 3011 (Slot 1), etc.
            // Math.floor(3001 / 10) = 300. Target prefix = 300 + index.
            if (Math.floor(userEquipmentInfo.equipmentInfo.id / 10) == (300 + index) && userEquipmentInfo.userEquipment.user_pet_id == 0) {
    ...
    updateBag(userEquipmentInfos: UserEquipmentInfo[]) {
        ...
        this.bagContentItemCount.string = "Số lượng:" + userEquipmentInfos.length + "/200";
    }
    ...
    clickBagItem() {
        ...
        this.detail_name.useSystemFont = true;
        this.detail_name.string = equipmentInfo.name;
        this.detail_starLevel.string = "Cấp Sao:+" + userEquipment.star_level;
        this.detail_strengthLevel.string = "Cường Hóa:+" + userEquipment.strength_level;
        this.detail_introduce.useSystemFont = true;
        ...
        for (let statusName in userEquipment.main_status) {
            ...
            this.detail_mainStatus.useSystemFont = true;
        ...
        for (let statusName in userEquipment.vice_status) {
            ...
            viceStatusItemCopy.getComponent(cc.Label).useSystemFont = true;

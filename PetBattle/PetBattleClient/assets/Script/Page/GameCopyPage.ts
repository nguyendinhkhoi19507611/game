...
export default class NewClass extends cc.Component {
    ...
    prepare(e: cc.Event.EventTouch) {
        ...
        if (labelName == "玲珑塔") labelName = "Tháp Linh Long";
        else if (labelName == "武斗场") labelName = "Võ Đấu Trường";
        else if (labelName == "元武库") labelName = "Kho Nguyên Vũ";
        this.currentTheme = labelName;
        this.panelReady_bannerLabel.useSystemFont = true;
        this.panelReady_bannerLabel.string = labelName;
        this.awardListArr.forEach(c => {
            c.active = false;
        });
        if (labelName == "Tháp Linh Long") {
            this.awardListArr[0].active = true;
        } else if (labelName == "Võ Đấu Trường") {
            this.awardListArr[1].active = true;
        } else if (labelName == "Kho Nguyên Vũ") {
            this.awardListArr[2].active = true;
        }
    }

    challenge() {
        Camper.getInstance().showToast("Chỉ có thể vào trong thời gian sự kiện");
    }
}

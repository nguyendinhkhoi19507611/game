        ...
        if (!ignoreEquipment) {
            for (let userEquipmentInfo of GlobalData.userEquipmentInfos) {
                if (userEquipmentInfo.userEquipment.user_pet_id == userPet.id) {
                    if (userEquipmentInfo.equipmentInfo.name.indexOf("Kính") != -1) {
                        userEquipmentInfos[0] = userEquipmentInfo;
                    } else if (userEquipmentInfo.equipmentInfo.name.indexOf("Vòng Tay") != -1) {
                        userEquipmentInfos[1] = userEquipmentInfo;
                    } else if (userEquipmentInfo.equipmentInfo.name.indexOf("Mũ") != -1) {
                        userEquipmentInfos[2] = userEquipmentInfo;
                    } else if (userEquipmentInfo.equipmentInfo.name.indexOf("Dây Chuyền") != -1) {
                        userEquipmentInfos[3] = userEquipmentInfo;
                    } else if (userEquipmentInfo.equipmentInfo.name.indexOf("Nanh") != -1) {
                        userEquipmentInfos[4] = userEquipmentInfo;
                    } else if (userEquipmentInfo.equipmentInfo.name.indexOf("Đá Tinh Tú") != -1) {

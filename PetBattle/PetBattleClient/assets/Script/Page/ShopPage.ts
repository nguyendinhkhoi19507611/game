...
export default class ShopPage extends cc.Component {
    ...
    refreshGoodsList() {
        GlobalData.player.call("ShopController.getGoodsList", [], (res) => {
            ...
            for (let i in goodsList) {
                ...
                copy.getChildByName("Name").getComponent(cc.Label).useSystemFont = true;
                ...
                copy.getChildByName("Count").getComponent(cc.Label).string =
                    "Còn lại:" + (item.max_buy - item.has_buy) +
                    "/" + item.max_buy;
    ...
    showModal(goodsItem: any) {
        ...
        if (goodsItem.goodsInfo.introduce) {
            modal.getChildByName("Introduce").getComponent(cc.Label).useSystemFont = true;
            modal.getChildByName("Introduce").getComponent(cc.Label).string = (goodsItem.goodsInfo.introduce as string).replace("\n", "");
        } else {
            modal.getChildByName("Introduce").getComponent(cc.Label).useSystemFont = true;
            modal.getChildByName("Introduce").getComponent(cc.Label).string = "Chưa có giới thiệu";
        }
        modal.getChildByName("Name").getComponent(cc.Label).useSystemFont = true;
        ...
        modal.getChildByName("Count").getComponent(cc.Label).string =
            "Còn lại:" + (goodsItem.max_buy - goodsItem.has_buy) +
            "/" + goodsItem.max_buy;
    ...
    update() {
        ...
        if (time > 0) {
            ...
            let output = (hour > 0 ? hour + "giờ" : "") + (minute > 0 ? minute + "phút" : "") + second + "giây";
            this.timeValue.string = output;
        } else {
            this.timeValue.string = "Đang làm mới";
        }
    }
}

...
export default class ResourceMgr extends cc.Component {
    ...
    onLoad() {
        cc.loader.loadResDir("", (completeCount, totalCount) => {
            ...
            Camper.getInstance().showLoading("Đang tải tài nguyên(" + rate + "%)")
        }, () => {
    ...
    dataLoading() {
        ...
        Camper.getInstance().showLoading("Đang tải dữ liệu(" + rate + "%)");
        if (this.dataComplete == this.dataTotal) {
            cc.director.preloadScene("Game", (completeCount, totalCount) => {
                Camper.getInstance().showLoading("Đang tải màn chơi(" + Math.floor((completeCount / totalCount) * 100) + "%)");
            }, () => {

...
export default class RankPage extends cc.Component {
    ...
    render(item: UserVsRank, index: number) {
        ...
        node.getChildByName("Nickname").getComponent(cc.Label).useSystemFont = true;
        node.getChildByName("Nickname").getComponent(cc.Label).string = item.nickname;
        node.getChildByName("Strength").getComponent(cc.Label).useSystemFont = true;
        node.getChildByName("Strength").getComponent(cc.Label).string = "Chiến Lực:\n" + item.strength;

...
export default class BagPage extends cc.Component {
    ...
    onLoad() {
        ...
        this.item_btns.children[1].on(cc.Node.EventType.TOUCH_END, () => {
            Camper.getInstance().showToast("Vật phẩm này không thể sử dụng trực tiếp");
        });
    }
    ...
    updateBag(userPropInfos: UserPropInfo[]) {
        ...
        this.bagPropCount.string = "Số lượng:" + userPropInfos.length + "/40";
    }
    ...
    clickTopBarBtn(e: cc.Event.EventTouch) {
        ...
        if (btnName == "全部") btnName = "Tất cả";
        else if (btnName == "消耗品") btnName = "Tiêu Hao";
        else if (btnName == "材料") btnName = "Nguyên Liệu";
        this.clearPanel();
        this.clearBag();
        if (btnName == "Tất cả") {
            this.updateBag(GlobalData.userPropInfos);

...
export default class PetPage extends cc.Component {
    ...
    clickDetailBtn(e: cc.Event.EventTouch) {
        ...
        if (btnName == "详情") btnName = "Chi Tiết";
        else if (btnName == "升级") btnName = "Nâng Cấp";
        else if (btnName == "血统") btnName = "Huyết Thống";
        else if (btnName == "突破") btnName = "Đột Phá";
        else if (btnName == "装备") btnName = "Trang Bị";

        if (btnName == "Chi Tiết") {
            this.updateDetailStatusList();
            this.updateSkillList();
        } else if (btnName == "Nâng Cấp") {
            new LevelUp(this);
        } else if (btnName == "Huyết Thống") {
            new BloodUp(this);
        } else if (btnName == "Đột Phá") {
            new BreakUp(this);
        } else if (btnName == "Trang Bị") {
            new Equipment(this);
        }
    }
    ...
    updateSkillList() {
        ...
        for (let i = 0; i < skills.length; i++) {
            let skill = skills[i];
            this.skillListContent.children[i].getChildByName("Name").getComponent(cc.Label).useSystemFont = true;
            this.skillListContent.children[i].getChildByName("Name").getComponent(cc.Label).string = (i + 1) + "、" + skill.name;
            this.skillListContent.children[i].getChildByName("Introduce").getComponent(cc.Label).useSystemFont = true;
    ...
    renderDisplay(userPet: UserPet, petInfo: PetInfo, strength: number) {
        ...
        this.petName.useSystemFont = true;
...
class LevelUp {
    ...
    init() {
        ...
        this.node.getChildByName("Btn").on(cc.Node.EventType.TOUCH_END, () => {
            if (this.userPetInfo.userPet.pet_level >= 100) {
                Camper.getInstance().showToast("Cấp thú cưng đã đầy");
                return;
            }
            if (this.prop_id) {
                if (GlobalData.getUserPropInfo(this.prop_id)) {
                    ...
                    Camper.getInstance().showToast("Cơm hộp kinh nghiệm này đã hết");
                }
            } else {
                Camper.getInstance().showToast("Vui lòng chọn cơm hộp kinh nghiệm");
            }
        });
    }
    ...
    updatePanel() {
        ...
        statusBaseList.children[0].getComponent(cc.Label).string = "Cấp cơ bản " + this.userPetInfo.userPet.pet_level;
        statusBaseList.children[1].getComponent(cc.Label).string = "Sinh mệnh：" + mainStatusBase[0];
        statusBaseList.children[2].getComponent(cc.Label).string = "Tấn công：" + mainStatusBase[1];
        statusBaseList.children[3].getComponent(cc.Label).string = "Phòng thủ：" + mainStatusBase[2];
        let statusNewList = this.node.getChildByName("StatusNewList");
        statusNewList.children[0].getComponent(cc.Label).string = "Cấp cơ bản " + (this.userPetInfo.userPet.pet_level + 1);
        statusNewList.children[1].getComponent(cc.Label).string = "Sinh mệnh：" + mainStatusNew[0];
        statusNewList.children[2].getComponent(cc.Label).string = "Tấn công：" + mainStatusNew[1];
        statusNewList.children[3].getComponent(cc.Label).string = "Phòng thủ：" + mainStatusNew[2];
        ...
        this.node.getChildByName("ProgressBG").getChildByName("Text").getComponent(cc.Label).useSystemFont = true;
        this.node.getChildByName("ProgressBG").getChildByName("Text").getComponent(cc.Label).string = "Kinh nghiệm：" + levelExp[0] + "/" + levelExp[1];
    }
}
class BloodUp {
    target: PetPage;
    ...
    init() {
        ...
        this.node.getChildByName("Btn").on(cc.Node.EventType.TOUCH_END, () => {
            if (this.userPetInfo.userPet.blood_level >= 10) {
                Camper.getInstance().showToast("Huyết thống đã đầy");
                return;
            }
            if (GlobalData.getUserPropInfo(this.prop_id)) {
                ...
                Camper.getInstance().showToast("Đá Linh Long đã dùng hết");
            }
        });
    }
    ...
    updatePanel() {
        ...
        let statusBaseList = this.node.getChildByName("StatusBaseList");
        statusBaseList.children[0].getComponent(cc.Label).string = "Cấp huyết thống " + bloodLevel;
        statusBaseList.children[1].getComponent(cc.Label).string = "Sinh mệnh tăng：" + (bloodLevel * 3) + "%";
        statusBaseList.children[2].getComponent(cc.Label).string = "Tấn công tăng：" + (bloodLevel * 3) + "%";
        statusBaseList.children[3].getComponent(cc.Label).string = "Phòng thủ tăng：" + (bloodLevel * 3) + "%";
        let statusNewList = this.node.getChildByName("StatusNewList");
        statusNewList.children[0].getComponent(cc.Label).string = "Cấp huyết thống " + (bloodLevel + 1);
        statusNewList.children[1].getComponent(cc.Label).string = "Sinh mệnh tăng：" + ((bloodLevel + 1) * 3) + "%";
        statusNewList.children[2].getComponent(cc.Label).string = "Tấn công tăng：" + ((bloodLevel + 1) * 3) + "%";
        statusNewList.children[3].getComponent(cc.Label).string = "Phòng thủ tăng：" + ((bloodLevel + 1) * 3) + "%";
        let statusUpList = this.node.getChildByName("StatusUpList");
        ...
        if (bloodExp < bloodExpMax / 3) {
            probability.getComponent(cc.Label).string = "Thấp";
        } else if (bloodExp < bloodExpMax * 2 / 3) {
            probability.getComponent(cc.Label).string = "Thường";
        } else {
            probability.getComponent(cc.Label).string = "Cao";
        }

        if (this.userPetInfo.userPet.blood_level >= 10) {
            this.node.getChildByName("ProgressBG").getChildByName("ProgressBar").getComponent(cc.Sprite).fillRange = 1;
            this.node.getChildByName("ProgressBG").getChildByName("Text").getComponent(cc.Label).string = "Huyết thống đã đầy";
        } else {
            this.node.getChildByName("ProgressBG").getChildByName("ProgressBar").getComponent(cc.Sprite).fillRange = bloodExp / bloodExpMax;
            this.node.getChildByName("ProgressBG").getChildByName("Text").getComponent(cc.Label).string = "Huyết thống：" + bloodExp + "/" + bloodExpMax;
        }
    }
}
class BreakUp {
    ...
    init() {
        ...
        this.node.getChildByName("Btn").on(cc.Node.EventType.TOUCH_END, () => {
            if (this.userPetInfo.userPet.break_level >= 10) {
                Camper.getInstance().showToast("Đột phá đã đầy");
                return;
            }
            if (this.userPetInfo.userPet.pet_level < ((this.userPetInfo.userPet.break_level + 1) * 10)) {
                Camper.getInstance().showToast("Cấp thú cưng không đủ");
                return;
            }
            let userPropInfo = GlobalData.getUserPropInfo(this.prop_id);
            if (!userPropInfo) {
                Camper.getInstance().showToast("Đá đột phá đã dùng hết");
                return;
            }
            if (userPropInfo.userProp.amount < this.userPetInfo.userPet.break_level + 1) {
                Camper.getInstance().showToast("Đá đột phá không đủ");
                return;
            }
            if (GlobalData.userInfo.coin < (this.userPetInfo.userPet.break_level + 1) * 10000) {
                Camper.getInstance().showToast("Tiền vàng không đủ");
                return;
            }
            if (this.userPetInfo.userPet.fragment == 0) {
                Camper.getInstance().showToast("Mảnh thú cưng không đủ");
                return;
            }
    ...
    updatePanel() {
        ...
        let statusBaseList = this.node.getChildByName("StatusBaseList");
        statusBaseList.children[0].getComponent(cc.Label).string = "Cấp đột phá " + breakLevel;
        statusBaseList.children[1].getComponent(cc.Label).string = "Sinh mệnh：+" + breakStatusBase[0];
        statusBaseList.children[2].getComponent(cc.Label).string = "Tấn công：+" + breakStatusBase[1];
        statusBaseList.children[3].getComponent(cc.Label).string = "Phòng thủ：+" + breakStatusBase[2];
        let statusNewList = this.node.getChildByName("StatusNewList");
        statusNewList.children[0].getComponent(cc.Label).string = "Cấp đột phá " + (breakLevel + 1);
        statusNewList.children[1].getComponent(cc.Label).string = "Sinh mệnh：+" + breakStatusNew[0];
        statusNewList.children[2].getComponent(cc.Label).string = "Tấn công：+" + breakStatusNew[1];
        statusNewList.children[3].getComponent(cc.Label).string = "Phòng thủ：+" + breakStatusNew[2];
        let statusUpList = this.node.getChildByName("StatusUpList");
        ...
        this.node.getChildByName("Tip").getChildByName("Label").getComponent(cc.Label).useSystemFont = true;
        this.node.getChildByName("Tip").getChildByName("Label").getComponent(cc.Label).string = "【Đột phá】yêu cầu cấp cơ bản đạt " + ((breakLevel + 1) * 10) + " cấp";
    }
}
class Equipment {
    target: PetPage;
    ...
    updatePanle() {
        ...
        for (let i = 0; i < 6; i++) {
            if (this.userPetInfo.userEquipmentInfos[i]) {
                ...
                item.getChildByName("RowName").getChildByName("Label").getComponent(cc.Label).useSystemFont = true;
                ...
                let names = ["Kính", "Vòng", "Nón", "Dây", "Răng", "Đá"];

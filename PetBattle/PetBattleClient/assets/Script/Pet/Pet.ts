...
export default class Pet extends cc.Component {
    ...
    releaseSkill(): any {
        if (this.isDead()) {
            return { pass: false, msg: "Thú cưng đã tử vong" };
        }
        if (this.isHurt()) {
            return { pass: false, msg: "Thú cưng đang bị tấn công" };
        }
        if (this.isControlled()) {
            return { pass: false, msg: "Thú cưng đang bị khống chế" };
        }
        if (this.isFullEnergy()) {
            this.startRound();
            ...
            this.skill();
            this.node.emit(Pet.EVENT_SKILL);
            return { pass: true, msg: "Thú cưng tung kỹ năng thành công" };
        }
        return { pass: false, msg: "Thú cưng không đủ năng lượng" };
    }
